...
const CreateBase = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post(`${process.env.REACT_APP_API}/api/v1/product/create-pizzaBase`, {name:name,price:price,qty:quantity});
            if (data?.success) {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.put(`${process.env.REACT_APP_API}/api/v1/product/update-pizzaBase/${selected._id}`, {name:updatedName,price:updatedPrice,qty:updatedQty})
            if (data?.success) {
