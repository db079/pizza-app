...
const CreateSauce = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post(`${process.env.REACT_APP_API}/api/v1/product/create-pizzaSauce`, {name:name,price:price,qty:quantity});
            if (data?.success) {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.put(`${process.env.REACT_APP_API}/api/v1/product/update-pizzaSauce/${selected._id}`, {name:updatedName,price:updatedPrice,qty:updatedQty})
            if (data?.success) {
    ...
    const handleDelete = async (id) => {
        try {
            const { data } = await axios.delete(`${process.env.REACT_APP_API}/api/v1/product/delete-pizzaSauce/${id}`)
            if (data?.success) {
    ...
    return (
        <Layout title={'pizzapp - add Sauce '}>
                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="mb-3">
                                    <input
                                        ...
                                        placeholder="Enter name of new Sauce"
                                        value={name}
